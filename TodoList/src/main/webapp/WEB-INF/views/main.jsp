...
<body>
	...
	<form action="/todo/add" method="post" id="addForm">
		...
		<div>
			<textarea row="3" cols="50" name="detail" 
	...
	<table id="todoList" border="1">
		<thead>
			<tr>
				<th>출력 번호</th> <%-- 페이지에서 보이는 용도의 단순 출력번호 --%>
		...
		<tbody>
			<c:forEach items="${todoList}" var="todo" varStatus="vs">
				<tr>
					...
					<td>
						<%-- 제목 클릭 시
							todoNo(고유 todo번호)를 테이터로 전송(제출)하여
					...
					<th>
						...
						<%-- todoComplete가 true라면 O 출력 --%>
						<c:if test="${not todo.todoComplete}">X</c:if> 
						<%-- todoComplete가 false라면 O 출력 --%>
					</th>
					<td>${todo.regDate}</td>
				</tr>
